...
/// 枚举方式实现
pub trait ToThirdPartyMsg1 {
    fn to_third_party_msg(&self) -> Result<ThirdPartyMsg, &dyn Error>;
}

impl ToThirdPartyMsg1 for Template {
    fn to_third_party_msg(&self) -> Result<ThirdPartyMsg, &dyn Error> {
        match self {
...
impl ToThirdPartyMsg1 for WeworkTpl {
    fn to_third_party_msg(&self) -> Result<ThirdPartyMsg, &dyn Error> {
        println!("WeworkTpl to_third_party_msg");
        Ok(ThirdPartyMsg::new_wework_msg())
    }
}

impl ToThirdPartyMsg1 for DingtalkTpl {
    fn to_third_party_msg(&self) -> Result<ThirdPartyMsg, &dyn Error> {
        println!("DingtalkTpl to_third_party_msg");
        Ok(ThirdPartyMsg::new_dingtalk_msg())
    }
}

impl ToThirdPartyMsg1 for FeishuTpl {
    fn to_third_party_msg(&self) -> Result<ThirdPartyMsg, &dyn Error> {
        println!("FeishuTpl to_third_party_msg");
        Ok(ThirdPartyMsg::new_feishu_msg())
    }
}